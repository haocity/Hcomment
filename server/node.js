      ...
      if (getobj.id) {
          ...
          console.log('查询id: '+getobj.id+'的评论');
           ...
           if (err){
            ...
            i++
            var d= rows;
            res.end('{"code":200,"data":'+JSON.stringify(d)+'}');
            db.end();
             console.log('关闭连接');
          });
      }else{
          res.end('{"code":0,"data":[{"text":"参数错误￣□￣｜｜"}]}');
      }
     
    }
    catch(error){
      console.log('发生错误了'+error)
      res.end('{"code":0,"data":[{"text":"请求出现错误￣□￣｜｜"}]}');
    }
}).listen(5225);
...
http.createServer(function (req, res) {
    ...
    req.on('end', function () {
      ...
      if(p.id&&p.cid&&p.user&&p.email&&p.text) { // 输出提交的数据
          ...
          db.query('CREATE TABLE IF NOT EXISTS `'+p.id+'`(`id` int(8) NOT NULL AUTO_INCREMENT,`user` char(30) NOT NULL, `email` char(40) NOT NULL,`weburl` char(40) DEFAULT NULL,`cid` char(20) NOT NULL,`time` char(30) NOT NULL,`text` text NOT NULL,PRIMARY KEY (`id`))');
          ...
          res.end(`{"code":200,"container":"发送成功","time":"${xtime}","success":1}`);
              console.log(`发送成功:${p.id}内容:${p.text}`);
              db.end();
              if(p.pm&&p.url&&p.title){
                        var mailOptions = {
                          from: "Hco评论<my@haotown.cn>", // 发件人+发件地址
                          to: p.pm, // 收件列表
                          subject: "你有一条新的回复", // 标题
